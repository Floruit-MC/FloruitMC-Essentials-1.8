...
public class WalkSpeedCommand extends BaseCommand {
    ...
    public void onWalkSpeed(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage("§c§lFLORUIT MC §f➤ §c✘ Utilize /walkspeed <valor|reset|clear> [jogador] para ajustar a velocidade.");
            return;
        }
        ...
        if (args[0].equalsIgnoreCase("reset") || args[0].equalsIgnoreCase("clear")) {
            ...
            if (target != player && !player.hasPermission("essentials.speed.walk.others")) {
                player.sendMessage("§c§lFLORUIT MC §f➤ §c✘ Você não tem permissão para alterar a velocidade de outros jogadores!");
                return;
            }

            target.setWalkSpeed(0.2F);
            target.sendMessage("§a§lFLORUIT MC §f➤ §a✔ Sua velocidade de caminhada foi resetada para o padrão.");
            target.playSound(target.getLocation(), Sound.ORB_PICKUP, 1.0F, 1.0F);

            if (target != player) {
                player.sendMessage("§e§lFLORUIT MC §f➤ §e✔ Velocidade de caminhada de " + target.getName() + " resetada para o padrão.");
            }
            return;
        }
        ...
        target.sendMessage("§a§lFLORUIT MC §f➤ §a✔ Sua velocidade ao andar foi alterada para " + quantity + ".");
        target.playSound(target.getLocation(), Sound.ORB_PICKUP, 1.0F, 1.0F);
        if (target != player) {
            player.sendMessage("§e§lFLORUIT MC §f➤ §e✔ Velocidade ao andar de " + target.getName() + " alterada para " + quantity + ".");
        }
    }
}